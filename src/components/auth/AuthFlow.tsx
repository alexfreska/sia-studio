...
export function AuthFlow() {
  ...
  return (
    <div className="flex-1 flex flex-col">
      {error && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 bg-bg-2 border border-danger/40 rounded-lg text-danger text-sm max-w-md text-center shadow-2xl">
          {error}
          <button
            type="button"
            onClick={() => setError(null)}
            className="ml-2 text-danger hover:text-fg btn-transition active:scale-[0.96]"
          >
            Dismiss
          </button>
        </div>
